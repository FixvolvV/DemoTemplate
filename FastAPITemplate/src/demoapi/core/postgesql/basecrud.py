from typing import Generic, Sequence, TypeVar, Union, List

from sqlalchemy import Result, select, inspect
...
class BaseCrud(Generic[T]):
    ...
    @classmethod
    async def adds(
        cls, session: AsyncSession, values: list[BaseModel]
    ) -> Union[T, None]:

        for value in values:
            values_dict = value.model_dump(exclude_unset=True)
            instanse = cls.model(**values_dict)
    ...
    async def update_one_by_id(
        cls, session: AsyncSession, id: str, new_values: BaseModel
    ) -> Union[T, None]:
